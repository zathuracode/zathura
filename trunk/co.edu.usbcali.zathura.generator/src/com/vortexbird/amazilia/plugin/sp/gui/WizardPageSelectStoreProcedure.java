			...
			public void widgetSelected(SelectionEvent e) {
				try {
					...
					if (isSchema == true && EclipseGeneratorUtil.connectionDriverTemplate.contains("Oracle") == true) {
						MetadataStoreProcedureHandler metadataStoreProcedureHandler = MetadataStoreProcedureHandlerFactory.getInstace().getHandler(
								EclipseGeneratorUtil.connectionDriverTemplate);
						java.util.List<String> storeProcedureNames = metadataStoreProcedureHandler.getStoredProcedureNames(ZathuraReverseEngineeringUtil
								.getSqlConnection().getConnection(), cmbCatlogSchema.getText(), txtStoreProceduresFilter.getText());
						...
						/*
						 * ITableInfo[]
						 * tableInfos=ZathuraReverseEngineeringUtil.
						 * getTables(cmbCatlogSchema.getText(), null,
						 * txtStoreProceduresFilter.getText()); for (ITableInfo
						 * tableInfo : tableInfos) {
						 * listAvailableStoreProcedures
						 * .add(tableInfo.getSimpleName()); }
						 */
					}
				} catch (Exception e2) {
					e2.printStackTrace();
