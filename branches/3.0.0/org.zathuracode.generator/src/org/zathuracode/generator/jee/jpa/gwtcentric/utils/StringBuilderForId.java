package org.zathuracode.generator.jee.jpa.gwtcentric.utils;
...
public class StringBuilderForId implements IStringBuilderForId {
		...
		if (metaData.getPrimaryKey().isPrimiaryKeyAComposeKey()) {
			...
			for (Field field2 : field) {
				...
				if (realType.equalsIgnoreCase("date")) {
					finalParam = finalParam + "(txt" + nameWithCapitalOnFirst + ".getValue())==null||(txt" + nameWithCapitalOnFirst
							+ ".getValue()).equals(\"\")?null:(" + realType + ")txt" + nameWithCapitalOnFirst + ".getValue(), ";
				} else {
					finalParam = finalParam + "(txt" + nameWithCapitalOnFirst + ".getValue())==null||(txt" + nameWithCapitalOnFirst
							+ ".getValue()).equals(\"\")?null:new " + realType + "(txt" + nameWithCapitalOnFirst + ".getValue().toString()), ";
				}
			...
			if (metaData.getPrimaryKey().getRealClassName().equalsIgnoreCase("date")) {
				finalParam = "(txt" + metaData.getPrimaryKey().getGetNameOfPrimaryName() + ".getValue())==null||(txt"
						+ metaData.getPrimaryKey().getGetNameOfPrimaryName() + ".getValue()).equals(\"\")?null:(" + metaData.getPrimaryKey().getRealClassName()
						+ ")txt" + metaData.getPrimaryKey().getGetNameOfPrimaryName() + ".getValue(), ";
			} else {
				finalParam = "(txt" + metaData.getPrimaryKey().getGetNameOfPrimaryName() + ".getValue())==null||(txt"
						+ metaData.getPrimaryKey().getGetNameOfPrimaryName() + ".getValue()).equals(\"\")?null:new "
	...
	public List<String> finalParamForIdVariablesAsList(List<MetaData> theMetaData, MetaData metaData) {
		...
		if (metaData.getPrimaryKey().isPrimiaryKeyAComposeKey()) {
			...
			for (Field field2 : field) {
				...
				finalParam2.add(Utilities.getInstance().ifcondition + name + "==null" + Utilities.getInstance().ifconditionClose
						+ Utilities.getInstance().throwExceptionNull + "\"" + name + "\"" + Utilities.getInstance().throwExceptionClose);

				finalParam2 = Utilities.getInstance().addVariablesValuesToListDependingOnDataTypeForID(finalParam2, field2, name, metaData.getComposeKey());
				...
				if (member.isPrimiaryKeyAComposeKey() == false) {
					...
					try {
						...
						finalParam2 = Utilities.getInstance().addVariablesValuesToListDependingOnDataType(finalParam2, member.getRealClassName(),
								member.getName(), member.getPrecision().toString(), member.getScale().toString(), member.getLength().toString());
					} catch (Exception e) {
					...
					for (int i = 0; i < params.length; i++) {
						...
						if (tmpFinalParam != null) {
							...
							if (!finalParam.contains(tmpFinalParam)) {
								...
								// try {
								// if (nullable == false) {
								finalParam2.add(Utilities.getInstance().ifcondition + tmp + "==null" + Utilities.getInstance().ifconditionClose
								...
								try {
									...
									finalParam2 = Utilities.getInstance().addVariablesValuesToListDependingOnDataType(finalParam2,
											primarySimple.getRealClassName(), tmp, primarySimple.getPrecision().toString(),
	...
	/* (non-Javadoc)
	 * @see org.zathuracode.generator.jee.jpa.gwtcentric.utils.IStringBuilderForId#finalParamForIdForViewForSetsVariablesInListGWT(java.util.List, org.zathuracode.metadata.model.MetaData)
	 */
	public List<String> finalParamForIdForViewForSetsVariablesInListGWT(List<MetaData> theMetaData, MetaData metaData) {
		...
		if (metaData.getPrimaryKey().isPrimiaryKeyAComposeKey()) {
			...
			for (Field field2 : field) {
				...
				// String idName = metaData.getPrimaryKey().getName();
				// txtMasCodigo.setValue(entity.getId().getMasCodigo());
				String tmp1 = (name.toString()) + " = " + metaData.getRealClassNameAsVariable() + ".get" + metaData.getPrimaryKey().getGetNameOfPrimaryName()
			...
			// txtCliCedula.setValue(entity.getCliCedula());
			finalParam = metaData.getPrimaryKey().getName();
			String tmp1 = finalParam.toString() + " = " + metaData.getRealClassNameAsVariable() + ".get" + (finalParam.substring(0, 1)).toUpperCase()
	...
	/* (non-Javadoc)
	 * @see org.zathuracode.generator.jee.jpa.gwtcentric.utils.IStringBuilderForId#finalParamForIdForDtoForSetsVariablesInListGWT(java.util.List, org.zathuracode.metadata.model.MetaData)
	 */
