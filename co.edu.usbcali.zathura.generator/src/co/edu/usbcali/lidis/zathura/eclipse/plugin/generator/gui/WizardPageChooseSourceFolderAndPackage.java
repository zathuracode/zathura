...
public class WizardPageChooseSourceFolderAndPackage extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		btnNewPackage.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				// "Select source folder container");
	...
	private void handleBrowseSourceFolder() {
		...
		// ResourceSelectionDialog(getShell(), project,
		// "Select source file container");
	...
	// Se usa para validar si la pagina se completo y poder activar el next o el
	// finish segun sea necesario
	private void validatePageComplete() {
		try {
			...
			// Valida si el paquete esta bien escrito porque el generador lo
			// crea si no existe
	...
	private void handleBrowseLibFolder() {
		...
		if (dialog.open() == Window.OK) {
			...
			if (result.length == 1) {
				...
				EclipseGeneratorUtil.libFolderPath = EclipseGeneratorUtil.workspaceFolderPath + txtLib.getText()
						+ co.edu.usbcali.lidis.zathura.generator.utilities.GeneratorUtil.slash;
			}
		}
	}
