...
public class GenerateXmlAndJavaFiles 
{
	...
	/**
	 * Generate xml.hbm files from given store procedures or functions
	 * in the given path.
	 * Generate an xml.hbm for each store Procedure
	 * 
	 * @param connection
	 *            oracle connection 
	 * @param strPath
	...
	public void generateProcedureXmlHbmFile(Connection connection,String strPath, List<String>procedureNames,MetadataStoreProcedureHandler metadataHandler) {
		try 
		{
			for (Iterator iter = procedureNames.iterator(); iter.hasNext();) {
				...
				ProcedureBean procedureBean = metadataHandler
						.getProcedureMetadata(connection, strProcedureName);
				...
				stringBuffer.append("\n");
				stringBuffer.append("	<sql-query name=" + "\""
						+ strProcedureName + "\"" + " callable=\"true" + "\">");
				stringBuffer.append("\n");

				List<String> columnNames = procedureBean.getColumnNames();
				List<String> columnTypeName = procedureBean
						.getColumnTypeNames();
				for (int i = 0; i < procedureBean.getIColumnCount(); i++) {
					stringBuffer.append("		<return-scalar column=" + "\""
							+ columnNames.get(i) + "\"" + " type=" + "\""
							+ columnTypeName.get(i) + "\"/>");
					stringBuffer.append("\n");
				}
	 ...
	 *            name of the package
	 */
	public void generateJavaClasses(Connection connection, String strPath,String strPackageName, List<String> procedureNames,MetadataStoreProcedureHandler metadataHandler) {
		
		TransformerFactory tFactory = TransformerFactory.newInstance();
		Transformer transformer;
		//MetadataHandler metadataHandler = MetadataHandlerFactory.getInstace().getHandler(GetConnection.getDataBase().getName());
		...
		{				
			for (Iterator iter = procedureNames.iterator(); iter.hasNext();) {
				...
				List<String> columnTypeName = procedureBean
						.getColumnTypeNames();
				List<String> isInOutArguments = procedureBean
						.getIsInOutArguments();
				List<String> argumentsTypeNames = procedureBean
						.getArgumentsTypeNames();
				StringBuffer stringBuffer = new StringBuffer();
				stringBuffer
				...
				stringBuffer.append("\n");
				stringBuffer.append("	<creationDate>" + date
						+ "</creationDate>");
				stringBuffer.append("\n");
				stringBuffer.append("	<className>"
						+ FormatCodeWithJalopy.formatNameFile(strProcedureName)
						+ "</className>");
				stringBuffer.append("\n");
				stringBuffer.append("	<procedureName>" + strProcedureName
						+ "</procedureName>");
				stringBuffer.append("\n");
				...
				for (int i = 0; i < procedureBean.getIColumnCount(); i++) {
					...
					stringBuffer.append("\n");
					stringBuffer.append("			<name>" + columnNames.get(i)
							+ "</name>");
					stringBuffer.append("\n");
					stringBuffer.append("			<type>" + columnTypeName.get(i)
							+ "</type>");
					stringBuffer.append("\n");
					stringBuffer.append("		</attribute>");
					stringBuffer.append("\n");
				...
				if (procedureBean.getIsInOutArguments().size() > 0) {
					for (int j = 0; j < isInOutArguments.size(); j++) {
						if (!isInOutArguments.get(j).equalsIgnoreCase("OUT")) {
							if (argumentsTypeNames.get(j).equalsIgnoreCase(
									"NUMBER")
									|| argumentsTypeNames.get(j)
											.equalsIgnoreCase("NUMERIC")) {
								stringBuffer.append("		<setQuery>");
								stringBuffer.append("\n");
								stringBuffer
										.append("			<setAttribute>setInteger("
												+ j + ",0)</setAttribute>");
								stringBuffer.append("\n");
								stringBuffer.append("		</setQuery>");
								stringBuffer.append("\n");
							...
							if (argumentsTypeNames.get(j).equalsIgnoreCase(
								...
								stringBuffer.append("		<setQuery>");
								stringBuffer.append("\n");
								stringBuffer.append("			<setAttribute>setLong("
										+ j + ",0)</setAttribute>");
								stringBuffer.append("\n");
								stringBuffer.append("		</setQuery>");
								stringBuffer.append("\n");
							...
							if (argumentsTypeNames.get(j).equalsIgnoreCase(
								...
								stringBuffer.append("\n");
								stringBuffer
										.append("			<setAttribute>setTimestamp("
												+ j
												+ ",new Date())</setAttribute>");
								stringBuffer.append("\n");
								stringBuffer.append("		</setQuery>");
								stringBuffer.append("\n");
							}
				...
				strProcedureName = FormatCodeWithJalopy
						.formatNameFile(strProcedureName);

				CreateFile.createFile(strPath + "/" + strPackageName,
						strProcedureName, "xml", stringBuffer.toString());

				// Generate beans
				transformer = tFactory.newTransformer(new StreamSource(GeneratorUtil.getstoreProcedureTemplates()+ "procedureBeanGenerator.xsl"));

				transformer.transform(new StreamSource(strPath + "/"
						+ strPackageName + "/" + strProcedureName + ".xml"),
						new StreamResult(new FileOutputStream(strPath + "/"
								+ strPackageName + "/" + "Bean"
								+ strProcedureName + ".java")));

				Utilities.commaSolution(strPath + "/" + strPackageName + "/"
						+ "Bean" + strProcedureName + ".java");
